...
try:
    from abstract.model.inference import main as main_inference
except:
    print('Had to use a relative import - am I on singularity?')
    from inference import main as main_inference
...
class CustomWandBTracker(GeneralTracker):
    ...
    def __init__(self, run_name: str, experiment_name: str):
        ...
        self.run = wandb.init(project=self.run_name, name=experiment_name, entity='lca-recipe')
        logger.info(f"Initialized WandB project {self.run_name} with name {self.experiment_name}")
        logger.info(
...
def parse_args():
    ...
    parser.add_argument(
        '--hf_model', default='primera', choices=['t5', 'primera']
    )
    parser.add_argument('-preemptible', default=False, action='store_true')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            'wide_beam', 'min_metric', 'max_metric', 'max_gap', 'min_gap', 'avg_margin',
        ],
    )
    parser.add_argument(
    ...
    if 'AMLT_OUTPUT_DIR' in os.environ and os.environ['AMLT_OUTPUT_DIR'] is not None:
        ...
        logger.info(f'Running on singularity. Saving results to {singularity_out} instead of {args.output_dir}')
        args.output_dir = os.environ['AMLT_OUTPUT_DIR']
        args.output_dir = os.path.join(os.environ['AMLT_OUTPUT_DIR'], 'weights')
        os.makedirs(args.output_dir, exist_ok=True)
    args.output_dir = os.path.join(args.output_dir, args.experiment)
    ckpt_dir = os.path.join(args.output_dir, 'best_ckpt')
    if args.preemptible and os.path.exists(ckpt_dir):
        print(f'We are restarting a pre-empted experiment on Singularity. Resuming from {ckpt_dir}...')
        args.resume_from_checkpoint = os.path.join(args.output_dir, 'best_ckpt')
    os.makedirs(args.output_dir, exist_ok=True)
...
def filter_contrast_sets(args, contrast_sets, train_uuids, target_n):
    def _get_metric_val(arr):
        return np.mean([arr[metric] for metric in args.contrast_metrics.split(',')])
    keep_sets = {k: v for k, v in contrast_sets.items() if k not in train_uuids}
    train_sets = {k: v for k, v in contrast_sets.items() if k in train_uuids}
    all_train_cset_uuids = list(train_sets.keys())
    if args.contrast_inter_sample_strategy == 'max_value':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            filt_key = -max([_get_metric_val(cs) for cs in train_sets[uuid]])
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'min_value':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            filt_key = min([_get_metric_val(cs) for cs in train_sets[uuid]])
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'max_margin':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            metric_vals = [_get_metric_val(cs) for cs in train_sets[uuid]]
            min_key = min(metric_vals)
            max_key = max(metric_vals)
            margin = max_key - min_key
            filt_key = -margin
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'min_margin':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            metric_vals = [_get_metric_val(cs) for cs in train_sets[uuid]]
            min_key = min(metric_vals)
            ...
            filt_key = margin
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'most_diverse':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            metric_vals = [_get_metric_val(cs) for cs in train_sets[uuid]]
            filt_key = -np.std(metric_vals)
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'least_diverse':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            metric_vals = [_get_metric_val(cs) for cs in train_sets[uuid]]
            filt_key = np.std(metric_vals)
            filt_keys.append(filt_key)
    elif args.contrast_inter_sample_strategy == 'most_likely':
        filt_keys = []
        for uuid in all_train_cset_uuids:
            if args.contrast_intra_sample_strategy == 'max_margin':
                likelihoods = np.mean([cs['bartscore'] for cs in train_sets[uuid]])
                metric_vals = [_get_metric_val(cs) for cs in train_sets[uuid]]
                metric_order = np.argsort(metric_vals)
                neg_idxs = metric_order[-args.max_num_negative]
                bartscores = np.mean([likelihoods[idx] for idx in neg_idxs])
            else:
                bartscores = np.mean([cs['bartscore'] for cs in train_sets[uuid]])
            filt_key = -bartscores
            filt_keys.append(filt_key)
    else:
        raise Exception(f'Unrecognized --contrast_inter_sample_strategy -> {args.contrast_inter_sample_strategy}')
    filt_keys_ordered = list(np.argsort(filt_keys))[:target_n]
    train_uuids_to_keep = [all_train_cset_uuids[i] for i in filt_keys_ordered]
    train_sets_filt = {k: v for k, v in train_sets.items() if k in train_uuids_to_keep}
    keep_sets.update(train_sets_filt)
    return keep_sets
...
def main():
    ...
    if args.debug:
        ...
        trackers = [CustomWandBTracker('chem_abstract', args.experiment)]
    ...
    def contrast_step(outputs, gold_labels, contrast_labels, contrast_cutoff):
        ...
        if args.contrast_objective == 'positive_teacher':
            contrast_inputs = {
